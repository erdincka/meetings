...
async def create_template(
    template_in: MeetingTemplateCreate, session: AsyncSession = Depends(get_db_session)
) -> APIResponse:
    ...
    return APIResponse(
        status="success", data=MeetingTemplateResponse.model_validate(new_template).model_dump()
    )


@router.get("/{template_id}", response_model=APIResponse)
async def get_template(
    template_id: UUID, session: AsyncSession = Depends(get_db_session)
) -> APIResponse:
    ...
    return APIResponse(
        status="success", data=MeetingTemplateResponse.model_validate(template).model_dump()
    )
