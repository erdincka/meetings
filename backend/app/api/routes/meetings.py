...
async def list_meetings(session: AsyncSession = Depends(database.get_db_session)) -> APIResponse:
    ...
    return APIResponse(
        status="success", data=[MeetingResponse.model_validate(m).model_dump() for m in meetings]
    )


@router.post("", response_model=APIResponse)
async def create_meeting(
    meeting_in: MeetingCreate, session: AsyncSession = Depends(database.get_db_session)
) -> APIResponse:
    ...
    if active_result.scalars().first():
        raise HTTPException(
            status_code=400,
            detail="Cannot start a new meeting. Another meeting is currently active.",
        )
            ...
            for doc_id_str in template.default_document_ids:
                ...
                if doc:
                    ...
                    new_doc = Document(
                        ...
                        library_scope="meeting"
                        if doc.library_scope == "company"
                        else doc.library_scope,
                ...
                if command == "start_meeting":
                    if meeting_id in active_meetings:
                        ...
                        await websocket.send_json(
                            {"type": "error", "content": "Meeting already running"}
                        )
                    else:
                        ...
                        async with database.async_session_maker() as session:
                            ...
                            if meeting:
                                ...
                                await session.commit()
                                logger.info(
                                    "meeting_status_updated_to_queued", meeting_id=meeting_id
                                )
